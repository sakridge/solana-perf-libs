...
void __host__ __device__ ge_scalarmult_vartime(ge_p2 *r, const unsigned char *a, const ge_p3 *A) {
...
int main(int argc, const char* argv[]) {
    ...
    get_time(&start);
    //for (int i = 0; i < 10; i++) {
        ge4_kernel<<<num_blocks, num_threads_per_block, 0, stream>>>
                (a_d,
                 As_d,
                 out_d,
                 total);
