        ...
        const uint8_t iv[CHACHA_IV_SIZE])
{
    ...
    input[0] =   load_4(&sigma[0]);
    input[1] =   load_4(&sigma[4]);
    input[2] =   load_4(&sigma[8]);
    input[3] =   load_4(&sigma[12]);
    ...
    input[12] = load_4(&iv[0]);
    input[13] = load_4(&iv[4]);
    input[14] = load_4(&iv[8]);
    input[15] = load_4(&iv[12]);
    ...
    while (len > 0) {
        ...
        chacha20_encrypt(input, buf.c, CHACHA_ROUNDS);

        for (size_t i = 0; i < todo; i++) {
            out[i] = inp[i] ^ buf.c[i];
        }
        for (size_t i = 0; i < 16; i++) {
            input[i] ^= load_4(&out[i * 4]);
        }
        out += todo;
        inp += todo;
        len -= todo;
